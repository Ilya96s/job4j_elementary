...
/**
 * Часто приходиться делать действия вида: если выполнено условие, то вернуть одно, иначе вернуть другое. По отношению к Optional это могут быть проверки если значение существует вернуть одно, иначе другое. Методы orElse(), orElseGet() позволяют опустить эти проверки. Если значение существует, то вернется значение, иначе то, что мы указали передали в методы
 *
 * Например,
 *
