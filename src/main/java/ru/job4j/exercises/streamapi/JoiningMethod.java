...
/**
 *
 * Для сборки текста мы использовали StringJoiner, или StringBuilder. Стримы также позволяют сложить элементы, если они являются строками, в единый текст. Для этого используется метод collect() и Collectors.joining(). Метод joining() принимает те же аргументы, что и StringJoiner
 *
 * Пример,
 *
