...
/**
 * необходимо реализовать метод boolean addNewElement(User u), который осуществит добавление нового элемента только при условии, что такого ключа в отображении еще нет. При этом метод должен вернуть булево значение - для этого необходимо проверить содержит ли отображение значение u. В отображении ключом является id пользователя, значением - объект типа User.
 *
